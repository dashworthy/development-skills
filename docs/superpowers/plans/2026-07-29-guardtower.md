...
Create `brief-template.md` — a `{{PLACEHOLDER}}` template mirroring `verity/skills/conducting-test-hardening/references/brief-template.md` in style. It must render: run id, PR reference, base and head sha, lenses run and lenses skipped, threshold; a summary count table (passed / dropped on evidence / discarded by gate); then one block per passed finding showing `{{ID}}`, lens, `{{COMPOSITE}}` with `{{VALUE}}`/`{{URGENCY}}` broken out, `{{TARGET_FILE}}:{{TARGET_LINE}}`, `in_diff`, claim, rationale, proposal, and — for reuse findings — `{{KIND}}`, `{{TIER}}`, `{{EXISTING_SOLUTION}}`, and `{{ADOPTION_COST}}`. Include HTML-comment instructions to omit reuse-only lines for non-reuse findings, as verity's template does.
...
**Interfaces:**
- Consumes: `{ forge: "github"|"gitlab", pr_number, repo, approved: [...], head_sha }` — `approved` being the in-scope subset of the arbitrator's `passed`.
...
5. **Comment body format** — `**guardtower <id>** (<composite>) — <claim>`, then the rationale, then the proposal, then a `value <n> · urgency <n>` footer so a reader can see how it scored. For reuse findings add a line naming `existing_solution`, and for tier 2 a line naming `adoption_cost`.
